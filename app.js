...
const APPS = [
  {
    ...
    solve_ja: "手持ちの食材からレシピを検索・提案。ただしプリセット20件の範囲。",
    solve_en: "Searches recipes from your ingredients. Limited to 20 preset recipes.",
    solve_level: 2,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "食材の期限を登録・色分け表示。「あと何日」が一目で分かる。",
    solve_en: "Register food with expiry dates. Color-coded alerts show days remaining.",
    solve_level: 4,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "「飲みました」ボタンで記録。飲んだかどうかが履歴で明確に。",
    solve_en: "\"Taken\" button records each dose. History makes it clear whether you took it.",
    solve_level: 5,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "忘却曲線に基づくSM-2アルゴリズムで、忘れかけた頃に自動で出題。",
    solve_en: "SM-2 algorithm based on forgetting curves. Auto-schedules reviews at optimal timing.",
    solve_level: 5,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "天気・気温・湿度・風速から蒸発散量を計算し、科学的に水やり要否を判定。",
    solve_en: "Calculates evapotranspiration from weather data to scientifically determine watering needs.",
    solve_level: 5,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "リアルタイムで音名とセント単位のずれを表示。高い/低いが数値で分かる。",
    solve_en: "Real-time display of note name and cent deviation. Sharp/flat shown numerically.",
    solve_level: 5,
    device: "mobile",
  ...
  },
  {
    ...
    solve_ja: "PDFの表を自動検出し、Excel/CSVに変換。手打ち転記が不要に。",
    solve_en: "Auto-detects PDF tables and converts to Excel/CSV. No more manual retyping.",
    solve_level: 5,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "銀行CSVを読み込んで自動カテゴリ分類。手入力不要でざっくり把握。レシート認識は未対応。",
    solve_en: "Import bank CSV for auto-categorization. No manual entry needed. Receipt OCR not supported.",
    solve_level: 3,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "CSVにSQLで直接集計。プリセットクエリ付きだが、SQL知識があるとより活用できる。",
    solve_en: "Run SQL directly on CSV files. Preset queries included, but SQL knowledge helps.",
    solve_level: 4,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "テンプレート3種からPDF自動生成。「書式を整える」時間は削減。「何を書くか」は自分で考える必要あり。",
    solve_en: "Auto-generates PDF from 3 templates. Saves formatting time, but content still needs human input.",
    solve_level: 2,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "銀行CSVから自動仕訳・カテゴリ分類。手書き帳簿からデジタルへ。分類ルールは初期37件。",
    solve_en: "Auto-categorizes bank CSV into tax categories. Replaces handwritten books. 37 initial rules.",
    solve_level: 4,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "在庫推移の追跡と低在庫アラート。棚卸・在庫管理は対応。原価計算は未実装。",
    solve_en: "Tracks inventory trends with low-stock alerts. Inventory management works. Cost calculation not implemented.",
    solve_level: 3,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "EXIFデータで日付・カメラ別に自動分類。重複検出や不要写真の判定は未対応。",
    solve_en: "Auto-sorts by date/camera using EXIF. Duplicate detection and quality judgment not implemented.",
    solve_level: 3,
    device: "desktop",
  ...
  },
  {
    ...
    solve_ja: "RSSフィードを統合してダイジェスト表示。「情報の多さ」は軽減。「出費」への対策は未対応。",
    solve_en: "Aggregates RSS feeds into a digest. Reduces info overload. Spending management not addressed.",
    solve_level: 2,
    device: "both",
  ...
  },
  {
    ...
    solve_ja: "4種テンプレートでフォーム入力→PDF出力。手書きからデジタルへ。複数利用者の一括管理は未対応。",
    solve_en: "4 templates with form input → PDF output. Digitizes handwritten records. Multi-patient management not supported.",
    solve_level: 4,
    device: "both",
  ...
  },
  {
    ...
    solve_ja: "問題入力→難易度分析→テスト用紙PDF生成。「PDF化」は自動化。「問題を考える」思考コストは削減されない。",
    solve_en: "Input questions → difficulty analysis → PDF generation. Automates formatting. Does NOT reduce the effort of creating questions.",
    solve_level: 2,
    device: "both",
  ...
  },
  {
    ...
    solve_ja: "GPX/FITファイルからペース・心拍・標高・ルート地図を詳細分析。km分割レポート付き。",
    solve_en: "Detailed analysis of pace, heart rate, elevation, and route map from GPX/FIT files. Per-km splits included.",
    solve_level: 5,
    device: "both",
